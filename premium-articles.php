<?php 
	...
	for ( $i=$start; $i <= $total_page; $i++ ) {
		$html = file_get_dom("https://oilprice.com/premium-articles/Page-".$i.".html");
		$bssnsQry = [];
	    foreach($html('.categoryArticle') as $element)
	    {
			...
			foreach ($html('#article-content') as  $data) {
				foreach ($data("p") as $caption) {
					if($caption->getPlainText() != 'By Julianne Geiger for Oilprice.com') {
						if($caption->getPlainText() !=  'More Top Reads From Oilprice.com:') {
